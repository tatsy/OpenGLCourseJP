...
TEX_FILE = os.path.join(os.path.dirname(__file__), "data", "cartoon_shading.png")
textureId = 0
...
def initTexture():
    ...
    image = np.array(Image.open(TEX_FILE), dtype='uint8')
    texHeight, texWidth, _ = image.shape

    textureId = glGenTextures(1)
    glBindTexture(GL_TEXTURE_2D, textureId)
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, texWidth, texHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, image.tobytes())
...
def paintGL():
    ...
    # テクスチャの有効化とシェーダへの転送
    # Enable a texture unit and transfer data to it
