...
def paintGL():
    ...
    if True:
        ...
        modelMat = np.eye(4)
        modelMat = np.dot(pyrr.matrix44.create_from_translation([-1.0, 0.0, 0.0]), modelMat)
        modelMat = np.dot(pyrr.matrix44.create_from_axis_rotation([0.0, 1.0, 0.0], np.radians(theta)), modelMat)
        modelMat = np.dot(pyrr.matrix44.create_from_axis_rotation([1.0, 0.0, 0.0], np.radians(theta) * 0.5), modelMat)
        modelMat = np.dot(pyrr.matrix44.create_from_scale([0.5, 0.5, 0.5]), modelMat)
        ...
        glUniform1i(uid, 1 if selectMode else -1)
    ...
    if True:
        ...
        modelMat = np.eye(4)
        modelMat = np.dot(pyrr.matrix44.create_from_translation([1.0, 0.0, 0.0]), modelMat)
        modelMat = np.dot(pyrr.matrix44.create_from_axis_rotation([0.0, 1.0, 0.0], np.radians(theta) * 2), modelMat)
        modelMat = np.dot(pyrr.matrix44.create_from_axis_rotation([1.0, 0.0, 0.0], np.radians(theta)), modelMat)
        modelMat = np.dot(pyrr.matrix44.create_from_scale([0.5, 0.5, 0.5]), modelMat)
        mvpMat = np.dot(modelMat, np.dot(viewMat, projMat))
...
def mouseEvent(window, button, action, mods):
    ...
    if action == glfw.PRESS:
        ...
        print("Mouse position: %d %d" % (cx, cy))
        print("Select object %d" % (byte[0]))
