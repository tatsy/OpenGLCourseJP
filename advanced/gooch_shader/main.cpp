...
// シェーダファイル
static const std::string SHADER_NAME = std::string(SHADER_DIRECTORY) + "render";
...
void initVAO() {
    ...
    std::string err, warn;
    bool success = tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, OBJECT_FILE.c_str());
    if (!warn.empty()) {
...
GLuint buildShaderProgram(const std::string &shaderName) {
    // シェーダの作成
    GLuint vertShaderId = compileShader(shaderName + ".vert", GL_VERTEX_SHADER);
    GLuint fragShaderId = compileShader(shaderName + ".frag", GL_FRAGMENT_SHADER);
...
// シェーダの初期化
void initShaders() {
    programId = buildShaderProgram(SHADER_NAME);
}
